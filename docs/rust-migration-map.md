...
| Command checks | `coding/command_*` | Rust command runner | Started with policy-gated argv runner, cwd scoping, timeout, bounded output, and CLI event recording; add richer approval/report integration next | Command tests pass and approval events emitted |
