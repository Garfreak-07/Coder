...
pub(crate) const PLANNER_USER_GUIDANCE_EVENT_KIND: &str = "planner.user_guidance.queued";
const PLANNER_USER_GUIDANCE_DELIVERED_EVENT_KIND: &str = "planner.user_guidance.delivered";
const PLANNER_USER_GUIDANCE_UNAPPLIED_EVENT_KIND: &str = "planner.user_guidance.unapplied";

#[derive(Debug, Clone)]
pub(crate) struct PlannerUserGuidance {
    sequence: u64,
    message: String,
}

pub(crate) fn queue_planner_user_guidance(
    ...
    let active_runs = state
        .active_run_controls
        .lock()
        .map_err(|_| "active run controls lock poisoned".to_owned())?;
    if !active_runs.contains_key(run_id.as_str()) {
        return Ok(None);
    }
    let queued = with_model_tool_event_lock(|| {
        let sequence = state
            .store
            .event_count(run_id)
        ...
        let event = CoderEvent::new(
            run_id.clone(),
            sequence,
            PLANNER_USER_GUIDANCE_EVENT_KIND,
            json!({
                "contract": "coder.planner_user_guidance.v1",
                "source": "planner-chat",
                "message": message,
                "priority": "now"
            }),
        ...
        state
            .pending_planner_guidance
            .lock()
            .map_err(|_| "planner guidance queue lock poisoned".to_owned())?
            .entry(run_id.to_string())
            .or_default()
            .push_back(PlannerUserGuidance {
                sequence,
                message: message.to_owned(),
            });
        Ok(Some(sequence))
    })
    .unwrap_or_else(|| Err("model tool event lock poisoned".to_owned()));
    drop(active_runs);
    queued
}

pub(crate) fn pending_planner_user_guidance_count(
    state: &crate::ApiState,
    run_id: &RunId,
) -> usize {
    state
        ...
        .lock()
        .ok()
        .and_then(|queues| queues.get(run_id.as_str()).map(|queue| queue.len()))
        .unwrap_or(0)
}

pub(crate) fn finalize_planner_user_guidance(
    state: &crate::ApiState,
    run_id: &RunId,
) -> Vec<String> {
    let pending = {
        let Ok(mut active_runs) = state.active_run_controls.lock() else {
            return Vec::new();
        };
        let Ok(mut queues) = state.pending_planner_guidance.lock() else {
            return Vec::new();
        };
        active_runs.remove(run_id.as_str());
        queues.remove(run_id.as_str()).unwrap_or_default()
    };
    if pending.is_empty() {
        return Vec::new();
    }

    let guidance_sequences = pending
        .iter()
        .map(|guidance| guidance.sequence)
        .collect::<Vec<_>>();
    let messages = pending
        .into_iter()
        .map(|guidance| guidance.message)
        .collect::<Vec<_>>();
    let _ = with_model_tool_event_lock(|| {
        let sequence = state.store.event_count(run_id).ok()? as u64 + 1;
        let event = CoderEvent::new(
            run_id.clone(),
            sequence,
            PLANNER_USER_GUIDANCE_UNAPPLIED_EVENT_KIND,
            json!({
                "contract": "coder.planner_user_guidance_delivery.v1",
                "source": "coder-server",
                "guidance_sequences": guidance_sequences,
                "delivery_status": "unapplied",
                "reason": "the workflow ended before another safe executor turn"
            }),
        );
        state.store.append_event(run_id, &event).ok()
    });
    messages
}

pub(crate) fn drain_planner_user_guidance_attachments(
    state: &crate::ApiState,
    run_id: &RunId,
) -> Vec<Value> {
    with_model_tool_event_lock(|| {
        let mut pending = state
            ...
            .lock()
            .ok()
            .and_then(|mut queues| queues.remove(run_id.as_str()))
            .unwrap_or_default();
        let mut attachments = Vec::new();
        let mut failed = std::collections::VecDeque::new();
        while let Some(guidance) = pending.pop_front() {
            let message = guidance.message.trim();
            if message.is_empty() {
                continue;
            }
            let Ok(sequence) = state
                .store
                .event_count(run_id)
                .map(|count| count as u64 + 1)
            else {
                failed.push_back(guidance);
                break;
            };
            let delivery_event = CoderEvent::new(
                run_id.clone(),
                sequence,
                PLANNER_USER_GUIDANCE_DELIVERED_EVENT_KIND,
                json!({
                    "contract": "coder.planner_user_guidance_delivery.v1",
                    "source": "coder-server",
                    "guidance_sequence": guidance.sequence,
                    "delivery_status": "delivered",
                    "delivery_channel": "model_tool_turn_attachment"
                }),
            );
            if state.store.append_event(run_id, &delivery_event).is_ok() {
                let prompt = wrap_in_system_reminder(&format!(
                    "The user added this requirement while the task was running. Apply it to the current task at the next safe point without discarding completed valid work:\n{message}"
                ));
                attachments.push(json!({
                    ...
                    "guidance_sequence": guidance.sequence,
                    "priority": "now",
                    "model_content": [{"type": "text", "text": prompt}]
                }));
            } else {
                failed.push_back(guidance);
                break;
        ...
        if !failed.is_empty() {
            if let Ok(mut queues) = state.pending_planner_guidance.lock() {
                let queued_while_draining = queues.entry(run_id.to_string()).or_default();
                failed.append(queued_while_draining);
                *queued_while_draining = failed;
            }
        }
        attachments
    })
