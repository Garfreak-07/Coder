...
use crate::model_tool_background_tasks::{
    is_claude_task_output_alias, model_tool_background_any_task_status,
    model_tool_background_command_status, model_tool_background_subagent_status,
    model_tool_stop_background_task, model_tool_task_output_options, TaskIdInput,
};
...
pub(crate) async fn execute_model_tool_request(
    ...
    match tool_name {
        ...
        "read_command_output" => {
            ...
            let options = model_tool_task_output_options(&request.tool_name, &request.input);
        ...
        "task_stop" => {
            let input = parse_input::<TaskIdInput>(&request.input)?;
            model_tool_stop_background_task(&state, &input).await
        }
        "patch_preview" => {
            let response =
        ...
        "read_subagent_status" => {
            ...
            let task_id = input.resolved_task_id()?;
            let options = model_tool_task_output_options(&request.tool_name, &request.input);
            if is_claude_task_output_alias(&request.tool_name) {
                model_tool_background_any_task_status(&state, &task_id, options).await
            } else {
...
async fn execute_sleep_model_tool(input: &Value) -> Value {
    ...
    json!({
        ...
        "claude_sources": [
            "src/query.ts Sleep drain check",
            "packages/builtin-tools/src/tools/SleepTool"
        ]
    })
}
