...
export const codingWorkbenchWorkflow: WorkflowSpec = {
  id: "default-planner-led-runtime",
  version: "0.3",
  name: "Planner-led Agent Workflow",
  description:
    "Planner decides. Executor changes only by order. Tester returns evidence. Runtime graph details stay hidden.",
  max_steps: 18,
  max_agent_calls: 18,
  max_tool_calls: 0,
  token_budget: 80000,
  agents: [
    ...
    },
    {
      ...
      name: "Executor Agent",
      role: "Executor Agent",
      goal: "Follow PlannerOrder and return only execution facts.",
      instructions: plannerInstructions,
      tools: [],
    ...
    },
    {
      ...
      name: "Tester Agent",
      role: "Tester Agent",
      goal: "Review execution evidence and return a TestResult.",
      instructions: plannerInstructions,
      tools: [],
  ...
  edges: [
    ...
    { from: "planner_loop", to: "planner_order", when: "planner_loop.should_continue == True", max_traversals: 3 },
